...
export function ChainOption({ chain }: { chain: AppKitChain }) { const config = getChainLogoConfig(chain); return <span className="flex min-w-0 items-center gap-3"><ChainLogo chain={chain} className="size-7"/><span className="min-w-0 truncate font-semibold leading-none text-slate-800 dark:text-lime-50">{config.label}</span></span>; }
export function SelectedChain({ chain }: { chain: AppKitChain }) { const config = getChainLogoConfig(chain); return <span className="flex min-w-0 flex-1 items-center gap-2.5 overflow-hidden"><ChainLogo chain={chain} className="size-7 shadow-none"/><span className="min-w-0 truncate font-semibold leading-none text-slate-900 dark:text-lime-50">{config.label}</span></span>; }
